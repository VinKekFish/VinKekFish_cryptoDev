 ...
 * 
 * Уменьшаем ёмкость губки до 2-х байтов. Пытаемся напрямую посмотреть на среднуюю длину гаммы
 * При этом используем не конструкцию "Sponge", а конструкцию "Overwrite" с перезаписью нулями внешнего состояния.
 * Она позволит быстрее забодать энтропию, т.к. часть данных всё время будет нулями перезаписана
 * Теоретическая же стойкость Overwrite совпадает со стойкостью губки
 * */
    unsafe class GammaOverwrite_VinKekFishTest: MultiThreadTest<GammaOverwrite_VinKekFishTest.SourceTask>
    {
        public GammaOverwrite_VinKekFishTest(ConcurrentQueue<TestTask> tasks): base(tasks, "GammaOverwrite_VinKekFishTest", new GammaOverwrite_VinKekFishTest.SourceTaskFabric())
        {
